...
export const addNoteToAnki = async (params: AddNoteParams) => {
  const result = await axios.post(ANKI_URL, {
    action: "addNote",
    version: 6,
    params: {
      note: {
...
export const syncAnki = async (): Promise<boolean> => {
  const syncResponse = await axios.post(ANKI_URL, {
    action: "sync",
    version: 6,
  });
...
export const fetchNotes = async (deck: string) => {
  const findNotesReponse = await axios.post<{ result?: number[]; error?: any }>(
    ANKI_URL,
    {
      action: "findNotes",
      version: 6,
  ...
  const notesInfoReponse = await axios.post<NotesInfoResult>(ANKI_URL, {
    action: "notesInfo",
    version: 6,
    params: {
